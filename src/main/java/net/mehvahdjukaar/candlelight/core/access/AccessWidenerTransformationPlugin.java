...
public final class AccessWidenerTransformationPlugin {
    ...
    public static void apply(Project project) {
        AccessTransformerExtension extension = project.getExtensions()
        ...
        project.afterEvaluate(p -> {
            p.getTasks().forEach(task -> {
                System.out.println("Task name " +
                        task.getPath() + " -> " + task.getClass().getName()
                );
            });
        });
    }
    ...
    private static Provider<File> generatedAccessTransformer(Project project) {
        return project.getLayout()
                ...
                .map(AccessWidenerTransformationPlugin::ensureParentDirsCreated);
    }
